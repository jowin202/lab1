...
E1_vec = [F[i]*L**3/(48*h1[i]*Iy_stab) for i in range(1,11)]
print(sum(E1_vec)/len(E1_vec))
...
f = open("result.tex", "w")
f.write("\\begin{align*}")
f.write("E_\\text{Holzstab} &= (" + str(round(e_stab/10**6)) + " \\pm " + str(round(e_stab_dev/10**6)) + ")~\\text{MPa} \\\\")
f.write("E_\\text{Holzbrett} &= (" + str(round(e_brett/10**6)) + " \\pm " + str(round(e_brett_dev/10**6)) + ")~\\text{MPa}")
f.write("\\end{align*}")
f.close()
